...
class PDF extends FPDF{
    ...
    function Header(){
        ...
        $this->Cell(80, 5, self::convert_codification('Nombre: '.($dataProvider['nombre'] == "" ? '' : $dataProvider['nombre'])),'',1,'L',0);

        $this->Cell(80, 5, self::convert_codification('Teléfono: '.($dataProvider['telefono'] == "" ? '' : $dataProvider['telefono'])),'',1,'L',0);

        $this->Cell(80, 5, self::convert_codification('Correo: '.($dataProvider['correo'] == "" ? '' : $dataProvider['correo'])),'',1,'L',0);

        $this->Cell(80, 5, self::convert_codification('Dirección: '.($dataProvider['direccion'] == "" ? '' : $dataProvider['direccion'])),'',1,'L',0);
...
$consulta = modeloPrincipal::consultar("SELECT
    ...
    E.fecha_entrada, U.cedula AS usuario
    FROM entrada AS E
