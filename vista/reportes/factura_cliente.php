...
if (!isset($_POST['UIDC']) || !isset($_POST['UIDV']) || !isset($_POST['UIDU'])){
    ...
    if (isset($_SESSION['id_usuario'])){
        ...
        header("location: http://localhost/sistema_de_control_de_ventas_y_inventario_la_chinita/vista/venta.php");
    }else{
        
        session_unset(); // remueve o elimina las variables de sesion
        session_destroy(); // Destruye la sesión actual
        header("location: http://localhost/sistema_de_control_de_ventas_y_inventario_la_chinita");
    }
}
...
if(mysqli_num_rows($detalles_venta_productos) > 0){
    while($row =  mysqli_fetch_array($detalles_venta_productos)){
        ...
        $pdf->Cell(26.66,4,convert_codification(modeloPrincipal::number_format_prices($row['precio_unidad_dolares'])." $"),0,0,'C');
        $pdf->Cell(26.66,4,convert_codification(modeloPrincipal::number_format_prices($row['precio_unidad_bolivares'])." bs"),0,1,'C');
...
if(mysqli_num_rows($detalles_venta_servicios) > 0){
    while($row =  mysqli_fetch_array($detalles_venta_servicios)){
        ...
        $pdf->Cell(26.66,4,convert_codification(modeloPrincipal::number_format_prices($row['precio_servicio_dolares'])." $"),0,0,'C');
        $pdf->Cell(26.66,4,convert_codification(modeloPrincipal::number_format_prices($row['precio_servicio_bolivares'])." bs"),0,1,'C');
...
$pdf->Cell(32,5,iconv("UTF-8", "ISO-8859-1",modeloPrincipal::number_format_prices($subTotal['sub_total_bs'])." bs"),0,0,'C');

$pdf->Ln(5);

$pdf->Cell(18,5,iconv("UTF-8", "ISO-8859-1",""),0,0,'C');
$pdf->Cell(22,5,iconv("UTF-8", "ISO-8859-1","IVA (16%)"),0,0,'C');
$pdf->Cell(32,5,iconv("UTF-8", "ISO-8859-1", modeloPrincipal::number_format_prices($subTotal['iva'])." bs"),0,0,'C');
...
$pdf->Ln(5);
    ...
    $pdf->Cell(32,5,iconv("UTF-8", "ISO-8859-1",modeloPrincipal::number_format_prices($subTotal['monto_total_bolivares'])." bs"),0,0,'C');
