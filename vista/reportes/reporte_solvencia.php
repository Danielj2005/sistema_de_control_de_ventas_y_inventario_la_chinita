...
class PDF extends FPDF{
    // Cabecera de página
    function Header(){
        $this->Image('../assets/img/bg-login.png', 1500, 5, 100, 100);
...
$pdf->AliasNbPages();
$pdf->AddPage('L',[1620 , 816]);
$pdf->SetAutoPageBreak(true, 5);
$pdf->SetTopMargin(5);
$pdf->SetLeftMargin(10);
$pdf->SetRightMargin(10);

$pdf->SetX(30); // posicion de los nombre de los campos en el eje x 
...
$pdf->Cell(70, 20, utf8_decode('ESTADO'),1,1,'C',1);

$pdf->Ln(0);
// $pdf->SetFont('Arial','B',10);
// $pdf->Cell(8, 1, utf8_decode('NOMBRES Y APELLIDOS'),1,0,'C',1);
// $pdf->Cell(3, 1, utf8_decode('CÉDULA'),1,0,'C',1);
// $pdf->Cell(8, 1, utf8_decode('CARRERA'),1,0,'C',1);
// $pdf->Cell(3, 1, utf8_decode('SEMESTRE'),1,0,'C',1);
// $pdf->Cell(3, 1, utf8_decode('TELÉFONO'),1,0,'C',1);
// $pdf->Cell(15, 1, utf8_decode('TÍTULO DE LIBRO'),1,0,'C',1);
// $pdf->Cell(3, 1, utf8_decode('COTA'),1,0,'C',1);
// $pdf->Cell(5, 1, utf8_decode('FECHA DE PRÉSTAMO'),1,0,'C',1);
// $pdf->Cell(5, 1, utf8_decode('OCUPACIÓN'),1,0,'C',1);
// $pdf->Cell(5, 1, utf8_decode('ESTADO'),1,1,'C',1);
...
if($fecha_inicio > $fecha_actual || $fecha_fin > $fecha_actual ){
    ...
    $pdf->SetFillColor(255,255,255);
    $pdf->SetDrawColor(0,0,0);
    $pdf->Cell(1320, 20, utf8_decode('DEBES SELECCIONAR OTRO RANGO DE FECHAS QUE SEA ANTERIOR A LA FECHA ACTUAL, VERIFICA E INTENTA NUEVAMENTE.'),1,0,'C',1);
    
    $pdf->Output("I","reporte-solvencia.pdf",true);
...
$datos = Main::Consulta("SELECT P.fecha_prestamo, P.semestre_solicitante, S.cedula, S.nombre,
S.apellido, S.telefono, L.cota, L.titulo, C.nombre_carrera, T.solicitante AS solicitante,
E.tipo_estado AS estado FROM prestamo AS P INNER JOIN libro AS L ON P.id_libro = L.id 
INNER JOIN solicitante AS S ON P.id_solicitante = S.id
INNER JOIN carrera AS C ON L.id_carrera = C.id
INNER JOIN estado AS E ON P.id_estado = E.id_estado
INNER JOIN tipo_solicitante AS T ON S.tipo_solicitante = T.id_solicitante 
WHERE P.id_estado = 4 AND S.tipo_solicitante != 3 AND P.fecha_prestamo 
BETWEEN '$fecha_inicio' AND '$fecha_fin' ORDER BY P.fecha_prestamo DESC");
...
if(mysqli_num_rows($datos) < 1){
    ...
    $pdf->SetFillColor(255,255,255);
    $pdf->SetDrawColor(0,0,0);
    $pdf->Cell(1320, 20, utf8_decode('NO HAY REGISTROS DISPONIBLES, SELECCIONA OTRO RANGO DE FECHAS.'),1,0,'C',1);

    $pdf->Output("I","reporte solvencia ({$fecha_inicio} -- {$fecha_fin}).pdf",true);
...
while($row = mysqli_fetch_assoc($datos)) {
    ...
    $pdf->Cell(70, 20, utf8_decode($row['estado']),1,1,'C',1);
