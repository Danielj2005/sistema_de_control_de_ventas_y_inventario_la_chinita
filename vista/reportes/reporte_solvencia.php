...
if($fecha_inicio > $fecha_actual || $fecha_fin > $fecha_actual ){
    ...
    $pdf->SetFillColor(255,255,255);
    $pdf->SetDrawColor(0,0,0);
    $pdf->Cell(1595, 20, utf8_decode('DEBES SELECCIONAR OTRO RANGO DE FECHAS QUE SEA ANTERIOR A LA FECHA ACTUAL, VERIFICA E INTENTA NUEVAMENTE.'),1,0,'C',1);
...
if(mysqli_num_rows($datos) < 1){
    ...
    $pdf->SetFillColor(255,255,255);
    $pdf->SetDrawColor(0,0,0);
    $pdf->Cell(1595, 20, utf8_decode('NO HAY REGISTROS DISPONIBLES, SELECCIONA OTRO RANGO DE FECHAS.'),1,0,'C',1);
...
while($row = mysqli_fetch_assoc($datos)) {
    ...
    $pdf->Cell(160, 20, utf8_decode(date('d-m-Y / h:i:a',strtotime($row['fecha_prestamo']))),1,0,'C',1);
