    ...
    for($i = 0; $i < COUNT($id_producto); $i++){
        ...
        <tr id="producto_<?= $id_producto[$i] ?>">
            ...
            <td class="col text-center">
                <input type="text" id="precio_producto_dolar_<?= $id_producto[$i] ?>" class="form-control precio_dolar_total bg-dark-subtle" name="precio_producto_dolar[]" value="<?= $datos_producto['precio_compra_dolar'];?>" readonly placeholder="precio en $" required>
            </td>
            <td class="col text-center">
                <input type="text" id="precio_producto_bolivar_<?= $id_producto[$i]?>" class="form-control precio_bolivar_total bg-dark-subtle" name="precio_producto_bolivar[]" value="<?= $datos_producto['precio_compra_bs']?>" readonly placeholder="precion en bss" required>
            </td>
        </tr>
    <?php }
}