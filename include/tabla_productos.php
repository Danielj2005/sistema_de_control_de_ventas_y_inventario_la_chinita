...
$consulta = modeloPrincipal::consultar("SELECT P.id_producto, P.nombre_producto, P.stock_actual,
    PS.cantidad, R.nombre AS representacion,
    M.nombre AS marca,
    ...
    ?>
        <tr id="tr_producto_<?= modeloPrincipal::encryptionId($mostrar['id_producto']) ?>" >
            <td class="col text-center " scope="col">
                <p class="text-primary text-wrap-balance"><?=  $mostrar['nombre_producto'].' '. $mostrar['marca'] ?>
                <span class=""><?= $mostrar['cantidad'].' '.$mostrar['representacion'] ?> </span>
                <span class="text-dark">En stock: <?= $mostrar["stock_actual"] == "0" || $mostrar["stock_actual"] == null ? "0" : $mostrar["stock_actual"] ?> </span> </p>
