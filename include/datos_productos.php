...
for($i = 0; $i < COUNT($id_producto); $i++){
    ...
    $datos_producto = mysqli_fetch_array(modeloPrincipal::Consultar("SELECT P.nombre_producto, P.id_producto, P.precio_compra_dolar, 
        P.precio_compra_bs, P.stock, PRE.nombre AS nombre_presentacion
        FROM producto AS P
    ...
    <tr id="producto_<?= $id_producto[$i] ?>">
        ...
        <?php 
            ...
            if($modulo == 'registrar_entrada') { ?> 
                ...
                <td class="col text-center">
                    <input type="text" class="form-control bg-dark-subtle" name="stock[]" value="<?= $datos_producto["stock"] ?>" readonly required>
                </td>
        ...
        <?php }
            ...
            if($modulo == 'agregar_servicio') { ?>
                ...
                <td class="col text-center" id="presentacion_<?= $id_producto[$i] ?>">
                    <input type="text" class="send_data form-control" name="presentacion_[]" disabled value="<?= $datos_producto['nombre_presentacion'] ?>" required>
                </td>
                <td class="col text-center" id="cantidad_<?= $id_producto[$i] ?>">
